...
const TopTrack: React.FC<Props> = ({ track, message }) => {
                        ...
                        return (
                            <div key={index} className='md:p-3 snap-start scroll-ml-6 flex-shrink-0 rounded-lg md:shadow-lg cursor-pointer md:hover:bg-gray-800/95 group w-44 md:w-auto md:bg-black/70'>
                                <div className='relative'>
                                    <img className='pointer-events-none rounded-lg shadow-2xl bg-black w-44 h-44 md:w-52 md:h-52' src={item?.album?.images[0]?.url} />
